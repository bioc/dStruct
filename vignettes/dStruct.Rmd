...
The input data to dStruct is a `list` object. Each element of the `list` should be a `data.frame` containing normalized reactivities. Currently, the preffered normalization method depends on the RNA structurome profiling technology. We have provided an implementation of the 2-8 % method for normalization (@low2010shape) via a function named `twoEightNormalize`, which is commonly used to normalize data from technologies such as SHAPE-Seq, Structure-Seq, etc.  
...
The function `plotDStructurome` takes in a list of `data.frame`s with reactivities and the result table from `dStructome` to save a PDF file showing the reactivities in a region. We illustrate this with the result for the `lai2019` data. Let us plot the reactivities for the gene with the lowest FDR value. We can identify it as follows.
```{r}
