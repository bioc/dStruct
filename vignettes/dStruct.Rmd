...
dStruct.guided(rl_predefined_regs[[1]], 
               reps_A = 2, reps_B = 1)
```
`dStruct.guided` returns a *p*-value for significance of differential PARS scores in the input region. Alternatively, all regions in `rl_predefined_regs` can be analyzed in one step using the wrapper function `dStructome`.
```{r}
